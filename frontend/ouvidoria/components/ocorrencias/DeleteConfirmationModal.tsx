...
const DeleteConfirmationModal = ({ ocorrencia, isSaving, onCancel, onConfirm }: DeleteConfirmationModalProps) => {
	if (!ocorrencia) {
		return null;
	}
	...
	return (
		<div className="fixed inset-0 z-40 flex items-center justify-center bg-black/60 px-4">
			<div className="w-full max-w-md space-y-6 rounded-3xl border border-neutral-200 bg-white p-8 text-center shadow-2xl">
				...
				<h2 className="text-lg font-semibold text-neutral-900">Tem certeza que deseja excluir?</h2>
				<p className="text-sm text-neutral-600">
					Essa ação não poderá ser desfeita. A ocorrência #{ocorrencia.cod_oco} será removida permanentemente.
				</p>
