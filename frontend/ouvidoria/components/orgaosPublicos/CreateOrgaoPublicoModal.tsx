...
const CreateOrgaoPublicoModal = ({ isOpen, formState, isSaving, onClose, onSubmit, onChange }: CreateOrgaoPublicoModalProps) => {
	if (!isOpen) {
		return null;
	}
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
			<div className="w-full max-w-2xl rounded-3xl border border-neutral-200 bg-white p-8 shadow-2xl">
				<div className="flex items-center justify-between">
					<h2 className="text-2xl font-semibold text-neutral-900">Adicionar órgão público</h2>
					<button
						type="button"
						onClick={onClose}
				...
				<form className="mt-6 space-y-5" onSubmit={onSubmit}>
					<div className="grid gap-4 md:grid-cols-2">
						<div className="space-y-2">
							<label htmlFor="novo-orgao-nome" className="text-sm font-semibold text-neutral-800">
								Nome do órgão
							</label>
							<input
								id="novo-orgao-nome"
						...
						<div className="space-y-2">
							<label htmlFor="novo-orgao-estado" className="text-sm font-semibold text-neutral-800">
								Estado
							</label>
							<input
								id="novo-orgao-estado"
								type="text"
					...
					<div className="space-y-2">
						<label htmlFor="novo-orgao-descricao" className="text-sm font-semibold text-neutral-800">
							Descrição
						</label>
						<textarea
							id="novo-orgao-descricao"
					...
					>
						{isSaving ? "Salvando..." : "Adicionar órgão"}
					</button>
				</form>
			</div>
		</div>
...
export default CreateOrgaoPublicoModal;
