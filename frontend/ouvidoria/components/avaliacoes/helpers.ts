...
export function buildAvaliacaoDisplay(
	...
	const moradorById = new Map(moradores.map((morador) => [morador.cpf_morador, morador]));
	...
	return avaliacoes.map((avaliacao) => {
		...
		const morador = moradorById.get(avaliacao.cpf_morador) ?? null;

		return {
			cod_avaliacao: avaliacao.cod_avaliacao,
			cod_servico: avaliacao.cod_servico,
			cpf_morador: avaliacao.cpf_morador,
